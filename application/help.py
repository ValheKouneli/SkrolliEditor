...
def getArticlesAmountCondition(amount=0, condition="(0=0)"):
    ...
    query = text(
        ...
        " LEFT JOIN (SELECT article_id, SUM(status) AS sum, COUNT(id) AS amount FROM picture GROUP BY article_id, sum, amount) PictureStatus ON PictureStatus.article_id = Article.id"
        " WHERE %s" % condition +\
